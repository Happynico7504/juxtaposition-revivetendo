...
messagesRouter.get('/new/:pid', async function (req, res) {
	...
	const newMessage = {
		screen_name: authCtx.user.mii.name,
		body: `${authCtx.user.mii.name} started a new chat!`,
		created_at: new Date(),
...
messagesRouter.get('/:message_id/:post_id/report', async function (req, res) {
	const { params, auth } = parseReq(req, {
		params: z.object({
			message_id: z.string(),
			post_id: z.string()
	...
	const conversation = await database.getConversationByID(params.message_id);
	if (!conversation || (auth().pid !== conversation.users[0].pid && auth().pid !== conversation.users[1].pid)) {
		return res.sendStatus(404);
	}

	const message = await POST.findOne({ id: params.post_id, community_id: params.message_id });
	if (!message) {
		return res.sendStatus(404);
	}
	...
	const props = {
		id: params.post_id,
		action: `/friend_messages/${params.message_id}/${params.post_id}/report`
	};
	res.jsxForDirectory({
...
messagesRouter.post('/:message_id/:post_id/report', upload.none(), async function (req, res) {
	...
	const duplicate = await database.getDuplicateReports(auth().pid, params.post_id);
	if (duplicate) {
		return res.redirect(`/friend_messages/${params.message_id}`);
	}
